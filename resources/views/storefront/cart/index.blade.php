...
@section('content')
    <section class="page-shell py-8">
        <div class="mb-8">
            ...
            <h1 class="mt-2 text-5xl">Your reading shortlist.</h1>
        </div>
                    ...
                    @foreach($items as $item)
                        <div class="panel p-5">
                            <div class="flex flex-col gap-5 md:flex-row md:items-center">
                                <div class="h-32 w-24 shrink-0 overflow-hidden rounded-2xl bg-gradient-to-br from-amber-100 to-slate-100">
                                ...
                                <div class="min-w-0 flex-1">
                                    <a href="{{ route('books.show', ['book' => $item['book']->slug]) }}" class="text-2xl font-semibold text-slate-950">{{ $item['book']->title }}</a>
                                ...
                                <div class="flex items-center gap-3">
                                    <form method="POST" action="{{ route('cart.update', $item['book']) }}" class="flex items-center gap-3">
                                        @csrf
                                        @method('PATCH')
                                        <input type="number" min="1" max="{{ $item['book']->stock }}" name="quantity" value="{{ $item['quantity'] }}" class="field-input w-24">
