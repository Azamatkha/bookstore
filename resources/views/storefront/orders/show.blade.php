...
@section('title', __('messages.order') . ' #' . $order->id . ' | ' . config('app.name', 'Bookstore'))

@section('content')
    <section class="page-shell py-6 sm:py-8">
        <div class="mb-8 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
                <p class="text-sm font-semibold uppercase tracking-[0.3em] text-slate-400">{{ __('messages.order_detail') }}</p>
                <h1 class="mt-2 text-3xl sm:text-4xl lg:text-5xl">{{ __('messages.order') }} #{{ $order->id }}</h1>
            </div>
        ...
        <div class="grid gap-6 lg:gap-8 lg:grid-cols-[1.1fr_0.9fr]">
            <div class="panel p-5 sm:p-6">
                <h2 class="text-2xl sm:text-3xl">{{ __('messages.items') }}</h2>
                <div class="mt-6 space-y-4">
                    @foreach($order->items as $item)
                        <div class="flex items-start justify-between gap-4 border-b border-slate-100 pb-4">
                            <div>
                                <p class="font-semibold text-slate-950">{{ $item->book_title }}</p>
                                <p class="text-sm text-slate-500">{{ __('messages.quantity') }}: {{ $item->quantity }}</p>
                            </div>
            ...
            <aside class="space-y-4">
                <div class="panel p-5 sm:p-6">
                    <h2 class="text-2xl sm:text-3xl">{{ __('messages.summary') }}</h2>
                    <div class="mt-6 space-y-3 text-sm text-slate-600">
                        ...
                        <div class="flex justify-between">
                            <span>{{ __('messages.placed') }}</span>
                            <span class="font-semibold text-slate-900">{{ $order->created_at->format('M d, Y H:i') }}</span>
                        </div>
                    </div>

                    <div class="mt-6 border-t border-slate-100 pt-6">
                        <p class="text-sm font-semibold uppercase tracking-[0.2em] text-slate-400">{{ __('messages.address') }}</p>
                ...
                <div class="panel p-5 sm:p-6">
                    <div class="flex items-center justify-between">
                        <p class="text-lg font-semibold text-slate-900">{{ __('messages.total_paid') }}</p>
                        <p class="text-2xl font-semibold text-slate-950 sm:text-3xl">@money($order->total_price)</p>
                    </div>
                    <a href="{{ route('orders.index') }}" class="btn-secondary mt-6 w-full">{{ __('messages.back_to_orders') }}</a>
                </div>
            </aside>
        </div>
    </section>
