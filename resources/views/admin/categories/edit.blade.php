...
@section('title', __('messages.edit_category_admin_title'))

@section('content')
    <div class="mb-8 flex items-center justify-between gap-4">
        <div>
            <p class="text-sm font-semibold uppercase tracking-[0.3em] text-slate-400">{{ __('messages.categories') }}</p>
            <h1 class="mt-2 text-5xl">{{ __('messages.edit') }} {{ $category->name }}.</h1>
        </div>
    ...
    <form method="POST" action="{{ route('admin.categories.update', $category) }}" enctype="multipart/form-data" class="panel p-6">
        ...
        <div class="mt-8 flex gap-3">
            <button type="submit" class="btn-primary">{{ __('messages.update_category_btn') }}</button>
