        ...
        <form method="POST" action="{{ route('checkout.store') }}" class="grid gap-8 lg:grid-cols-[1fr_0.9fr]">
            ...
            <div class="panel p-6">
                <div class="grid gap-6">
                    ...
                    <div>
                        <label class="field-label" for="payment_type">Payment type</label>
                        <select id="payment_type" name="payment_type" class="field-select" required>
                            @foreach($paymentTypes as $paymentType)
                                <option value="{{ $paymentType->value }}" @selected(old('payment_type') === $paymentType->value)>
                                    {{ $paymentType->value === 'card' ? 'Card (Wallet Balance)' : $paymentType->label() }}
                                </option>
                            @endforeach
                        </select>
                        @error('payment_type')
                        ...
                        <p class="mt-2 text-xs text-slate-500">
                            ...
                            <a href="{{ route('wallet.show') }}" class="font-semibold text-amber-700">Top up</a>
                        </p>
                    </div>
                </div>
            </div>
